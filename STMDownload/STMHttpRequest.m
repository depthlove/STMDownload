...
@interface STMHttpRequest()<ASIHTTPRequestDelegate,ASIProgressDelegate>
{
...
// 全部暂停的时候  request 并不 retain 它们的代理，所以已经释放了代理 而之后 request 完成了，这将会引起崩溃。大多数情况下，如果你的代理即将被释放，你一定也希望取消所有request，因为你已经不再关心它们的返回情况了，所以得在STMHttpRequest 这个类的 dealloc 里面加上一个 [request clearDelegatesAndCancel];
// 代理类的dealloc函数
- (void)dealloc
{
